...
# ────────────────────────────────────────────────────────────────
# 🔄 최근 N일 vs 역대 N일: 일자별 비교 꺾은선그래프
# ────────────────────────────────────────────────────────────────
# 1) 최근 N일 데이터 정리
recent_plot = (
    recent_df[["날짜", "최고기온(℃)", "최저기온(℃)"]]
    .sort_values("날짜")
    .reset_index(drop=True)
)
recent_plot["날짜_str"] = recent_plot["날짜"].dt.strftime("%Y-%m-%d")

# 2) MM-DD → 역대 평균 매핑 (hist_avg는 이미 위에서 구함)
recent_plot["역대 평균 최고기온(℃)"] = recent_plot["날짜"].dt.strftime("%m-%d").map(
    hist_avg["최고기온(℃)"]
)
recent_plot["역대 평균 최저기온(℃)"] = recent_plot["날짜"].dt.strftime("%m-%d").map(
    hist_avg["최저기온(℃)"]
)
...
fig_compare = px.line(
    ...
    color="구분",
    markers=True,
    title=f"최근 {day_range}일 실제 vs 역대 평균 기온 비교",
    labels={"날짜_str": "날짜"},
)
